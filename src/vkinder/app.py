...
class APP:
    ...
    def progress_bar(data, func, name, one=False, bar_len = 50):
        ...
        for i, value in enumerate(data):
            ...
            print(f'{name} [{"*"*p_bar + " "*(bar_len - p_bar)}] {i} of {data_len} {datetime.now() - start}\r', end='')
            if one:
                func(value)
            else:
    ...
    def settings_to_dict(self):
        return {
            ...
            'intersection': tuple(self.intersection_params.items())\
                if isinstance(self.intersection_params, dict) else self.intersection_params
                }

    def save_settings_to_file(self):
        with open(f'profile_{self.user_id}.json', mode='w', encoding='utf-8') as file:
            json.dump(self.settings_to_dict(),
    ...
    def load_settings_from_vk(self):
        ...
        search_fields = ['city', 'country', 'hometown', 'sex', 'has_photo', 'religion']
        search_fields_with_fix = {'universities': 'university', 'schools': 'school', 'career': 'company'}
        intersection_fields = [
            'universities', 'schools', 'status', 'activities', 'interests', 'music', 'movies',\
            'tv', 'books', 'games', 'about', 'quotes', 'career', 'military', 'langs'
            ]
        match_fields = [
            'verified', 'sex', 'city', 'country', 'home_town', 'has_photo', 'has_mobile', 'common_count',\
            'occupation', 'relation', 'can_post', 'can_see_all_posts', 'can_see_audio', 'can_write_private_message',\
            'can_send_friend_request', 'is_hidden_from_feed', 'blacklisted', 'blacklisted_by_me', 'political',\
            'religion', 'inspired_by', 'people_main', 'life_main', 'smoking', 'alcohol'
            ]

        search_params = {field: value for field, value in settings.items() if field in search_fields}
        ...
        match_params = {field: value for field, value in settings.items()\
            if field in match_fields and field not in search_params}

        intersection_params = {field: value for field, value in settings.items()\
            if field in intersection_fields and value and field not in search_params}
    ...
    def out(self, users):
        ...
        with open(file_name, mode='w', encoding='utf-8') as file:
            json.dump(
                [{f'https://vk.com/{user["domain"]}': self.get_top_three_profile_photos(user['id'])}\
                    for user in users], 
    ...
    def check_settings(self):
        ...
        for param in obligatory_search_params:
            if param not in self.search_params:
                self.search_params[param] = input(f'Нехватает {param}. Введите значение: ')
        if 'group_id' not in self.search_params:
            load = input('Нехватает групп. Введите списк id групп через запятую или "load" для загрузки.')
            if load == 'load':
                self.search_params['group_id'] = self.vk.get_groups(self.user_id)
            else:
                self.search_params['group_id'] = load.split(', ')


    def make_flat_searc_params(self):
        arrays = [product([key], value) for key, value in self.search_params.items()\
